...
public class Airborne : CCBase
{
	...
	// 에어본 중에 넉백 당하는 경우
	// 넉백중에 에어본 하는 경우
	IEnumerator DoAirborn(CreatureState lastState)
	{
		Vector3 originalPosition = Owner.Anim.transform.localPosition;
		...
		for (float t = 0; t < halfTickTime; t += Time.deltaTime)
		{
			...
			Owner.Anim.transform.localPosition = Vector3.Lerp(originalPosition, upPosition, normalizedTime);
			yield return null;
		}
		...
		{
			float normalizedTime = t / halfTickTime;
			Owner.Anim.transform.localPosition = Vector3.Lerp(upPosition, originalPosition, normalizedTime);
			yield return null;
		}

		Owner.Anim.transform.localPosition = originalPosition;
